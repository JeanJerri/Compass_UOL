...
args = getResolvedOptions(sys.argv, ['JOB_NAME','S3_INPUT_PATH_MOVIES', 'S3_INPUT_PATH_SERIES','S3_TARGET_PATH'])
...
spark_df_movies = spark_df_movies.filter(
    ((col("genero").contains("Fantasy")) | (col("genero").contains("Sci-Fi")) | (col("genero").contains("\\N"))) &
    ((col("anoLancamento") > 2000) | (col("anoLancamento") == "\\N"))
)

spark_df_series = spark_df_series.filter(
    ((col("genero").contains("Fantasy")) | (col("genero").contains("Sci-Fi")) | (col("genero").contains("\\N"))) &
    ((col("anoLancamento") > 1983) | (col("anoLancamento") == "\\N")) &
    ((col("notaMedia") > 6.9) | (col("notaMedia") == "\\N")) &
    ((col("numeroVotos") > 9) | (col("numeroVotos") == "\\N"))
)

spark_df_movies = spark_df_movies.select("id", "tituloPincipal", "tituloOriginal", "anoLancamento", "genero")
spark_df_series = spark_df_series.select("id", "tituloPincipal", "tituloOriginal", "anoLancamento", "anoTermino", "tempoMinutos", "genero", "notaMedia", "numeroVotos")

spark_df_movies = spark_df_movies.dropDuplicates()
spark_df_series = spark_df_series.dropDuplicates()

spark_df_movies = spark_df_movies.withColumnRenamed("tituloPincipal", "tituloPrincipal")
spark_df_series = spark_df_series.withColumnRenamed("tituloPincipal", "tituloPrincipal")
...
if spark_df_movies.rdd.isEmpty():
    ...
    dynamic_df_movies = DynamicFrame.fromDF(spark_df_movies, glueContext, "dynamic_frame_name")
...
if spark_df_series.rdd.isEmpty():
    ...
    dynamic_df_series = DynamicFrame.fromDF(spark_df_series, glueContext, "dynamic_frame_name")
